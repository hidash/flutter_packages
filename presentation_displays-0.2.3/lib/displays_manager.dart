...
class DisplayManager {
  ...
  FutureOr<String?> getWidth(int index, {String? category}) async {
    List<Display> displays = await getDisplays(category: category) ?? [];
    String? width="0";
    if (index >= 0 && index <= displays.length){
      width = displays[index].width.toString();
      return width;
    }
  }

  FutureOr<String?> getHeight(int index, {String? category}) async {
    List<Display> displays = await getDisplays(category: category) ?? [];
    String? height="0";
    if (index >= 0 && index <= displays.length) height = displays[index].height.toString();
    return height;
  }
